...
class Strategy():
            ...
            if i != self.active_player_unum:
                ...
                if not teammate_pos is None:
                    if i in point_preferences:
                        distance = np.sum(
                            (teammate_pos - point_preferences[i]) ** 2)
                        if(distance > 0.3):
    ...
    def potential_fields_pathfinding(self, start_pos, goal_pos=None):
        ...
        resulting_force = attraction_force + repulsion_force + boundary_correction + \
            self.central_field_correction(
                start_pos) - self.damping_coefficient * self.previous_force
    ...
    def calculate_attraction(self, current_pos, goal_pos):
        ...
        boundary_proximity_boost = 1.5 if np.linalg.norm(
            self.apply_boundary_correction(current_pos)) > 0.5 else 1.0
        attraction_multiplier = 1.0 if self.ball_dist > 0.5 else 1.5

        attraction_force = self.goal_attraction_coefficient * direction_to_goal * \
            decay_factor * attraction_multiplier * boundary_proximity_boost
    ...
    def calculate_repulsion(self, current_pos):
        ...
        for opponent_pos in self.opponent_positions:
            if opponent_pos is not None:
                direction_from_obstacle = np.array(
                    current_pos) - np.array(opponent_pos)
    ...
    def apply_boundary_correction(self, current_pos):
        ...
        if abs(current_pos[0]) > 13:
            ...
            correction_force[0] = -1.0 / \
                (distance_to_boundary_x + 0.1) * np.sign(current_pos[0])

        if abs(current_pos[1]) > 8:
            distance_to_boundary_y = 10 - abs(current_pos[1])
            correction_force[1] = -1.0 / \
                (distance_to_boundary_y + 0.1) * np.sign(current_pos[1])
    ...
    def next_position_to_startat(self, target, ball, myposition, startat, buffer_distance=0.4, angle_threshold=78):
        ...
        if abs(angle_degrees) < angle_threshold:
            perpendicular_direction = np.array(
                [-target_to_ball[1], target_to_ball[0]]) / norm_target_to_ball
        ...
        for i in range(num_players):
            if self.teammate_positions[i] is not None and self.teammate_positions[i][0] > position[0] + 1.5:
                current_distance = self.distance(
                    self.teammate_positions[i], position)
