...
class Agent(Base_Agent):
    ...
    def move(self, target_2d=(0, 0), orientation=None, is_orientation_absolute=True,
        ...
        if avoid_obstacles:
            ...
            distance_to_final_target = np.linalg.norm(
                np.array(target_2d) - r.loc_head_position[:2])

        self.behavior.execute("Walk", target_2d, True,
                              orientation, is_orientation_absolute, distance_to_final_target)
    ...
    def think_and_send(self):
        ...
        strategyData.teammate_positions = [
            pos if pos is not None else default_pos for pos in strategyData.teammate_positions]
        strategyData.opponent_positions = [
            pos if pos is not None else default_pos for pos in strategyData.opponent_positions]
            ...
            if strategyData.play_mode == self.world.M_PLAY_ON:
                ...
                if ball_at_center:
                    ...
                    if strategyData.robot_model.unum == 5:
                        ...
                        self.kickTarget(
                            strategyData, strategyData.mypos, KICK_TARGET_POS)
                    else:
    ...
    def customDribbleAndShoot(self, strategyData):
        ...
        if my_pos[0] > X_POSITION_TO_SHOOT and my_pos[1] < 3 and my_pos[1] > -3 and ball_dist < 0.5:
    ...
    def select_skill(self, strategyData):
        ...
        if my_unum in HUNTER_UNUMS:
            ...
            if not np.array_equal(target_opp_pos, default_pos):
                ...
                drawer.annotation(tuple(target_opp_pos),
                                  f"HUNTING OPP 5", drawer.Color.red, "exploit")
            else:
                strategyData.my_desired_position = OPPONENT_5_SPOT
                drawer.annotation(tuple(OPPONENT_5_SPOT),
                                  f"HUNTING OPP 5 (SPOT)", drawer.Color.orange, "exploit")
            ...
            drawer.annotation(tuple(player_1_target),
                              f"RIGHT BLOCK (FACE -Y)", drawer.Color.red, "exploit")
            ...
            distance_to_target = np.linalg.norm(
                np.array(strategyData.mypos) - player_3_target)

            if distance_to_target < DISTANCE_TOLERANCE:
                desired_orientation_degs = final_orientation_degs
                drawer.annotation(tuple(player_3_target),
                                  f"LEFT BLOCK (TURNING)", drawer.Color.green, "exploit")
            else:
                desired_orientation_degs = strategyData.GetDirectionRelativeToMyPositionAndTarget(
                    player_3_target)
                drawer.annotation(tuple(player_3_target),
                                  f"LEFT BLOCK (MOVING)", drawer.Color.green, "exploit")
            ...
            desired_orientation_degs = M.target_abs_angle(
                strategyData.mypos, OUR_NET_POS)
            drawer.annotation(tuple(player_4_target),
                              f"FRONT BLOCK (FACE NET)", drawer.Color.blue, "exploit")
            ...
            desired_orientation_degs = M.target_abs_angle(
                strategyData.mypos, OUR_NET_POS)
            drawer.annotation(tuple(player_2_target),
                              f"BACK BLOCK (FACE NET)", drawer.Color.cyan, "exploit")
        ...
        if strategyData.active_player_unum == strategyData.robot_model.unum:
            drawer.annotation((0, 10.5), "Role Assignment Phase",
        ...
        valid_teammates = [
            pos for pos in current_teammates if pos is not None and not np.array_equal(pos, default_pos)]

        num_teammates_seen = len(valid_teammates)
        if num_teammates_seen < 5:
            dummy_pos = np.array([-100.0, -100.0])
            padded_teammates = valid_teammates + \
                [dummy_pos] * (5 - num_teammates_seen)
        else:
        ...
        if len(padded_formation) < 5:
            dummy_pos = np.array([-100.0, -100.0])
            padded_formation = padded_formation + \
                [dummy_pos] * (5 - len(padded_formation))
        ...
        drawer.line(strategyData.mypos,
                    strategyData.my_desired_position, 2, drawer.Color.blue, "target line")
        ...
        if strategyData.active_player_unum == strategyData.robot_model.unum:
            drawer.annotation((0, 10.5), "Dribbling to Goal",
                              drawer.Color.green, "status")
        else:
            drawer.clear_player()
